...
def carregar_padroes():
    if os.path.exists('padroes.json'):
        try:
            ...
            st.error(f"🚨 ERRO GRAVE: O arquivo 'padroes.json' está mal formatado ou quebrado! O sistema não consegue ler os defeitos. Detalhe técnico: {e}")
            return []
    else:
        st.error("🚨 ERRO: O arquivo 'padroes.json' não foi encontrado no servidor.")
        return []
...
try:
    # A key v27 quebra a cache antiga e força o site a atualizar
    arquivo = st.file_uploader("", key="analise_v27")
    ...
    if arquivo is not None:
        ...
        if not (nome_arquivo.endswith('.ips') or nome_arquivo.endswith('.txt')):
            ...
            # Só continua se os padrões carregarem corretamente
            if padroes:
    ...
    st.error(f"🚨 Ocorreu um erro inesperado no sistema: {erro_geral}. Envie um print desta tela ao suporte!")

st.write("")
st.write("")
st.write("")
with st.expander("🔒"):
    senha = st.text_input("Chave de Acesso:", type="password", key="senha_admin")
    if senha == "iphoneecia":
